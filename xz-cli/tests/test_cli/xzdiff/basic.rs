...
// Test xzdiff exit codes for equal and different contents.
add_test!(exit_codes_equal_and_different, async {
    ...
    let out = fixture.run_cargo("xzdiff", &[&same1_xz, &same2_xz]).await;
    assert!(out.status.success());

    let out = fixture.run_cargo("xzdiff", &[&same1_xz, &diff_xz]).await;
    assert!(out.status.code() == Some(1));
});

// Test xzdiff with a single operand.
//
// This tests that xzdiff correctly compares the compressed file to the original file when
...
add_test!(single_operand_comparison, async {
    ...
    let out = fixture.run_cargo("xzdiff", &[&file_xz]).await;
    assert!(out.status.success());
});
