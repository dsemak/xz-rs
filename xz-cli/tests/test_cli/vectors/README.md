...
0. Introduction
    ...
    This directory contains a collection of files to test handling of
    .xz, .lzma (LZMA_Alone), and .lz (lzip) files in decoder
    implementations. Many of the files have been created by hand with a
    hex editor, thus there is no better "source code" than the files
    themselves. All the test files and this README may be distributed
    under the terms of the BSD Zero Clause License (0BSD).

    In this repository, the bundled fixtures are grouped into the
    subdirectories xz/, lzma/, and lz/. The file names listed below are
    base names; combine them with the matching subdirectory when looking
    up a specific fixture on disk.
