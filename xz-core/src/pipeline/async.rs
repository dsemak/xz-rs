...
mod tests {
    ...
    async_test!(error_empty_compressed_input, {
        let mut decompressed = Vec::new();
        let options = DecompressionOptions::default();
        let result = decompress_async([].as_slice(), &mut decompressed, &options).await;
    ...
    async_test!(decompress_grows_pending_input_buffer, {
        let mut compressed = Vec::new();
        let compression_options = CompressionOptions::default();
        let _ = compress_async(SAMPLE, &mut compressed, &compression_options)
            .await
            .unwrap();
        ...
        let summary = decompress_async(reader, &mut decompressed, &decompression_options)
            .await
            .unwrap();
    ...
    async_test!(truncated_stream_with_pending_bytes_returns_data_error, {
        let mut compressed = Vec::new();
        let compression_options = CompressionOptions::default();
        let _ = compress_async(SAMPLE, &mut compressed, &compression_options)
            .await
